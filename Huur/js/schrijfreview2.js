...
function submitReview() {
    ...
    reviewName = document.getElementById("reviewName").value.trim();

    // Check character limits
    if (reviewText.length > 255) {
        alert("Review text should not exceed 255 characters.");
        return;
    }

    if (reviewName.length > 50) {
        alert("Review name should not exceed 50 characters.");
        return;
    }
...
function fetchReviews() {
    const reviewsRef = database.ref("reviews/2");
